...
web_spring/
  ...
  make_data.php           генератор данных из confmap.svg (73 системы, 109 туннелей)
...
- **73 системы** — `id="star_XXX"` (пятиугольники, центр `sodipodi:cx/cy`),
  ...
  код = `strtoupper(XXX)`.
- **104→109 гиперканалов** — Inkscape-коннекторы (`inkscape:connection-start/end="#star_XXX"`),
...
- В каждой системе — объект STAR (для PointLight и цвета свечения) и OORT
  (`oort_radius=30` — лимит zoom-out).
...
- **Фаза 1 — планетарные системы**: генерация внутреннего наполнения систем
  (звёзды/планеты/пояса/OORT) для каждой из 73 систем; текстуры планет —
  из `../web/media/` (симлинк или копия). Проверка: полёт по нескольким системам.
