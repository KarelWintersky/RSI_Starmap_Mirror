...
web_spring/
  ...
  make_data.php           генератор топологии из confmap.svg (73 системы, 109 туннелей) → bootup.json
  generate_systems.php    генератор лоции систем (bootup.json → star-systems/*.json + _index.json)
...
### Источник: `../confmap.svg` (Фаза 0)

Данные берутся из корневого `confmap.svg` — исходника карты (слои Stars&Channels + Labels),
а не из боевого API Star Citizen:

- **73 системы** — `id="star_XXX"` (пятиугольники, центр `sodipodi:cx/cy`),
  русское имя из слоя Labels `id="st_XXX"` (наборы `star_*` и `st_*` совпадают 1-в-1),
  код = `strtoupper(XXX)`.
- **109 гиперканалов** — Inkscape-коннекторы (`inkscape:connection-start/end="#star_XXX"`),
  55 пунктирных (`stroke-dasharray` → `dashed=true`), рисуются `LineDashedMaterial`.
  5 коннекторов используют легаси-имена звёзд, которых нет в SVG → резолвятся через
  `STAR_ALIAS`: Фоллхейм→Роканнон, Хальдемар→KCD-X4 (`star_kcdx4`), Ахерон→V-AMD5
  (`star_acheron`, именно так подписан в слое Labels).
- `TARGET_SPAN=300` — весь размах карты ужимается в 300 мировых единиц (центрировано).

### Лоция систем (`generate_systems.php`)

Пока — **случайная генерация** (метод `genSystemLayout()`); в будущем заменится
чтением описаний систем из текстового файла. Настройки — `../config.confmap.php`
(`return ['generation' => [...]]`): `planets_min/max`, `belt_chance`, `generate_oort`,
`jump_radius_au`, `oort_radius_au`, `seed` (null = случайно, число = детерминированно).

На каждую систему генерится:
- STAR (обязателен движку: PointLight, цвет свечения из bootup `color`);
- 2..6 планет (обозначения A..Z) на орбитах «как в Солнечной системе» —
  0.39/0.72/1.0/1.52/5.2/9.54 АЕ, размеры как у Меркурия/Венеры/Земли/Марса/
  Юпитера/Сатурна (км, с jitter ±15%), `appearance` случайный (атмосфера
  для PLANET_BLUE/GREEN);
- с шансом `belt_chance` вместо 5-й планеты (слот Юпитера) — пояс астероидов
  (тип ASTEROID_BELT, ширина ~1.4 АЕ);
- гиперканалы в районе орбиты 6-й планеты (`jump_radius_au`, 9.54) — по одному
  JUMPPOINT на каждый туннель системы (код `{SYS}.JUMP.{DEST}`);
- OORT в районе «Плутона» (`oort_radius_au`, 39.5) — опция `generate_oort`;
  `oort_radius` системы = этому значению (лимит zoom-out в системном виде).

Пунктирность туннелей пока не используется (в будущем — фаза канала: активный/пассивный).
...
- **[done] Фаза 0 — каркас на реальных данных**: three.js vendored, план, генератор
  данных из `confmap.svg`, сервер, вертикальный срез: 2D → 3D → система (73 системы,
  109 туннелей). Проверено headless chromium (CDP): инициализация, двойной клик по
  звезде → 3D, клик по звезде → система, двойной клик по пустоте → zoom out (лимит OORT),
  без ошибок в консоли; `make lint` зелёный.
- **[done] Фаза 1a — случайная лоция систем**: `generate_systems.php` генерит
  star-systems/*.json из bootup.json (настройки — `config.confmap.php`); проверено
  CDP на реальном датасете. Схемы раскладки планет пользователь заготовит позже —
  тогда случайная генерация (`genSystemLayout()`) заменится чтением текстовых описаний.
- **Фаза 1b — текстуры планет**: подключить текстуры из `../web/media/`
  (симлинк или копия), выбор по типу/внешности планеты. Проверка: полёт по нескольким системам.
- **Фаза 2 — красота**: перенос оригинальных GLSL-шейдеров (звёзды, джамп-поинты,
  гиперканал-туннель `DustGoTrhu`, атмосферы), модели `.dae` через Collad NVIDIA Tesla P40aLoader,
...
```
make spring-data                       # топология (confmap.svg) + случайная лоция систем
